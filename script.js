let taxiFareApiUrl = 'https://taxifare.lewagon.ai/predict'; // API - запит до бекенду
const centralCoordinates = [25.9403400,48.2914900 ]; // starting position


mapboxgl.accessToken = 'pk.eyJ1IjoiYW5kcmlpMjAyMyIsImEiOiJjbGhxZGNybngwZmc1M2ptZnF3bDA2eGFyIn0.UKFK1lQmlueuXFdBTf6zLQ'; // дані Апі, сайт на якому я зарєгався і взяв токен аутентифікації в сайті мапбокс
...
const displayMap = (start, stop) => {
  ...
  function getRoute(end) {
    const url = 'https://api.mapbox.com/directions/v5/mapbox/driving/' + start[0] + ',' + start[1] + ';' + end[0] + ',' + end[1] + '?steps=true&geometries=geojson&access_token=' + mapboxgl.accessToken; // add sum money

    const req = new XMLHttpRequest(); //request from const
...
// inputs
const pickupAutocomplete = () => { // array function
